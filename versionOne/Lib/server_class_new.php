...
class Server {
	 ...
	 * @param $port The port for the master socket (default: 5003)
	 */
	function Server($address = '127.0.0.1', $port = 5003, $verboseMode = false) {
				...
				if($socket == $this->master) {
					...
					if($client) {
						...
						if(!$client->getHandshake()) {
							if($handShake = $this->handshake($client, $data)) {
